...
void initUart(GPIO_TypeDef * GPIOx, USART_TypeDef * USARTx, uint8_t pin_rx, uint8_t pin_tx, uint32_t baudrate){
    ...
    GPIOx->OSPEEDR |= (GPIO_Speed_50MHz << pin_rx * 2) | (GPIO_Speed_50MHz << pin_tx * 2);
    // Set PUPDR register
    ...
    else if(USARTx == UART4 || USARTx == UART5 || USARTx == USART6){
        GPIOx->AFR[pin_rx >> 3] |= GPIO_AF_UART456 << (pin_rx % 8) * 4;
        GPIOx->AFR[pin_tx >> 3] |= GPIO_AF_UART456 << (pin_tx % 8) * 4;
    }
