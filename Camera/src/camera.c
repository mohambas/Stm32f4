...
RESPONSE setBaudRate(uint8_t* mode){
    ...
    if(checkResponse(responseBuffer, SET_PORT) == 1){
        ...
        // Reinit a camera with new baudrate
        if(mode[1] == B_115200_H && mode[2] == B_115200_L)
            initUart(UART_GPIO, UART_INTERFACE, UART_RX, UART_TX, UART_BAUDRATE_115200);
        else if(mode[1] == B_57600_H && mode[2] == B_57600_L)
            initUart(UART_GPIO, UART_INTERFACE, UART_RX, UART_TX, UART_BAUDRATE_57600);
    }
    else
